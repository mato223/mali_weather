...
df_tombouctou_hourly_2023['date'] = pd.to_datetime(df_tombouctou_hourly_2023['date'])

df_tombouctou_hourly_2023['time_of_day'] = ['Day' if is_day else 'Night' for is_day in df_tombouctou_hourly_2023['is_day']]
data_avg_temp = df_tombouctou_hourly_2023.groupby(['time_of_day', df_tombouctou_hourly_2023['date'].dt.date])['temperature_2m'].mean().reset_index()
...
temp_humd_fig = px.scatter(df_tombouctou_hourly_2023, x='temperature_2m', y='relative_humidity_2m', title='Corrélation entre Température et Humidité Relative',
                 labels={'temperature_2m': 'Température (°C)', 'relative_humidity_2m': 'Humidité Relative (%)'})


###############

facette_day_night = px.scatter(df_tombouctou_hourly_2023, x='temperature_2m', y='relative_humidity_2m', color='is_day', facet_col='time_of_day',
                 labels={'temperature_2m': 'Température (°C)', 'relative_humidity_2m': 'Humidité relative (%)'},
...
polar_wind_fig.add_trace(go.Barpolar(
    r=df_tombouctou_hourly_2023['wind_speed_10m'],
    theta=df_tombouctou_hourly_2023['wind_direction_10m'],
    marker_color='rgba(75, 101, 132, 0.8)',
    name='Vitesse du Vent'
))
df_tombouctou_hourly_2023
polar_wind_fig.update_layout(
    ...
    polar=dict(
        radialaxis=dict(
            visible=True,
            range=[0, df_tombouctou_hourly_2023['wind_speed_10m'].max()]
        )),
    showlegend=False
)

################
df_tombouctou_hourly_2023['month'] = df_tombouctou_hourly_2023['date'].dt.strftime('%B')

month_fig = px.bar(df_tombouctou_hourly_2023.groupby('month')['temperature_2m'].mean().reset_index(),
...
relation_fig = px.scatter_polar(df_tombouctou_hourly_2023, r='temperature_2m', theta='wind_direction_10m',
